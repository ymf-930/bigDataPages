  ...
  var option = {
    ...
    legend: {
      ...
      formatter: function (name) {
        var target;
        var percent;
        for (var i = 0; i < panelData3.length; i++) {
          if (name == panelData3[i].name) {
            target = "" + (panelData3[i].value) + "条"
            percent = "" + (panelData3[i].percent) + "%"
            break;
          }
        }
        return '{a|' + name + '}' + '{value|' + target + '}{percent|' + percent + '}'
...
(function () {
  // 实例化对象
  var dom = document.getElementById("pieChart4");
  var myChart = echarts.init(dom);
  var panelData4 = [
    {icon: 'circle', name: '信用良好', value: 7500, percent: 33.3},
    {icon: 'circle', name: '守信', value: 4500, percent: 16.2},
      ...
      formatter: function (name) {
        ...
        for (var i = 0; i < panelData4.length; i++) {
          if (name == panelData4[i].name) {
            target = "" + (panelData4[i].value) + "条"
            percent = "" + (panelData4[i].percent) + "%"
            break;
          }
        }
        return '{a|' + name + '}' + '{value|' + target + '}{percent|' + percent + '}'
